...
sappia come eseguire `grg-profile`, perché senza profilo di progetto le figure parlano per luoghi
...
4. Procedi senza domande di configurazione. La severità si deriva dalla criticità dichiarata nel
   ...
   il default operativo resta `normal` e il setup propone `grg-profile`.
...
- **Non crea `{project-root}/_bmad/memory/grl-shared/`.** La crea `grg-profile` alla prima
...
- **Non imposta una stanza di default.** Le tre figure restano nella stanza principale insieme
  agli agenti BMM; in più `grg-setup` installa stanze tematiche richiamabili con
...
3. **Proponi `grg-profile` e, se l'utente accetta, eseguilo subito.** È il passo che rende utile
