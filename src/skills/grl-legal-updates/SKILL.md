...
di controllo → i due gate `bmad-review` sul report esistente. Se mancano giurisdizione o periodo
...
Non invocare `bmad-domain-research`: è uno shim deprecato che inoltra alla stessa skill e non
aggiunge capacità.

Passa a Deep Recon un brief che contenga giurisdizione, periodo, settori/parole chiave e questo
obiettivo: costruire un registro di aggiornamenti legali correnti, con priorità alle dimensioni
“regole del gioco” e “cambiamenti pendenti” del pack domain. Usa il preset `standard`,
`validation = max`, ricerca web live e `red_team = on`: il controllo massimo è il default. Dividi
gli assistenti per fonte/atto; il lead deduplica e compila la matrice di copertura.
...
Dopo che esiste il report e il suo source appendix, esegui due invocazioni separate di
**`bmad-review`**, in contesto fresco e in questa sequenza:
...
2. **Gate indipendente di completezza.** Dopo le correzioni, invoca di nuovo `bmad-review` in
   ...
   un editore diverso o una versione ufficiale successiva. Lo stesso URL non conta due volte.
   Verifica che `as_of` sia reale, che nulla fuori periodo sia diventato un risultato corrente e
   che ogni finding marcato `vigente-confermato` soddisfi tutti i requisiti del reference.
...
senza spiegazione, se una pagina non è accessibile o se `bmad-review` non è installato/disponibile.
In questi casi il finding resta `unverified`/`disputed`/`blocked` e il report dichiara il limite,
la fonte o l'arco mancante e il prossimo controllo necessario. La data di accesso da sola non
dimostra che una pagina sia la versione più recente.

## Output

Conserva il report prodotto da Deep Recon nella sua cartella di ricerca e restituisci in
conversazione un riepilogo breve. Il report deve avere almeno:
...
- registro dei due gate `bmad-review`, con esito e correzioni;
...
non può colmare una fonte mancante né cambiare lo stato ufficiale di un atto. Se Deep Recon non è
disponibile, usa una ricerca web live diretta con lo stesso brief e la stessa mappa delle fonti;
se manca anche il web, restituisci solo ciò che è verificabile dai materiali forniti e dichiara il
limite. Non presentare come attuale una norma ricordata.
